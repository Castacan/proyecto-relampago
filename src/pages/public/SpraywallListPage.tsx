    ...
    <div className="min-h-screen bg-fondo">
      ...
      <div className="max-w-md mx-auto w-full px-4 py-5">
        ...
        <Link
          ...
          className="w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl bg-superficie border border-zinc-700/50 text-zinc-300 font-bold text-sm mb-5 hover:bg-superficie-alta hover:text-texto-principal transition-all active:scale-[0.98]"
        >
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${
              gradeFilter === null ? 'bg-primario text-texto-en-acento' : 'bg-superficie-alta text-zinc-400 hover:text-texto-principal'
            }`}
          >
            Todos
          </button>
            ...
            {(['todas', 'enviadas', 'pendientes'] as SendFilter[]).map(f => (
              <button
                ...
                className={`flex-1 py-2 rounded-xl text-xs font-bold capitalize transition-all ${
                  sendFilter === f ? 'bg-primario text-texto-en-acento' : 'text-zinc-400 hover:text-texto-principal'
                }`}
              >
                {f}
              </button>
