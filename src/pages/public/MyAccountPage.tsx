  ...
  if (!session?.user) {
    return (
      <div className="min-h-screen bg-fondo flex flex-col items-center justify-center p-8 text-center">
        ...
        <h1 className="text-texto-principal font-black text-xl tracking-tight mb-2">Tu cuenta</h1>
        <p className="text-zinc-500 text-sm mb-8">Inicia sesión para ver tus puntos y estadísticas.</p>
        <button
          onClick={() => setAuthSheetOpen(true)}
          className="px-8 py-4 bg-primario hover:bg-primario-hover text-texto-en-acento font-bold rounded-2xl text-sm active:scale-95 transition-all"
        >
          Iniciar sesión
        </button>
  ...
  return (
    <div className="min-h-screen bg-fondo flex flex-col">
      ...
      <div className="shrink-0 flex items-center justify-between px-5 py-4 border-b border-zinc-800/40">
        ...
        <h1 className="text-texto-principal font-black text-base tracking-tight">Mi cuenta</h1>
        <button
      ...
      <div className="flex-1 max-w-md mx-auto w-full px-5 py-6 space-y-6">
        ...
        <Link
          to="/spraywall"
          className="flex items-center justify-between px-4 py-3.5 bg-superficie border border-zinc-800/60 rounded-2xl hover:border-zinc-700 transition-all"
        >
          <span className="text-texto-principal font-semibold text-sm">Ver Spraywall</span>
        ...
        <div>
          ...
          <div className="bg-superficie rounded-2xl border border-zinc-800/60 p-4 space-y-3">
            <input
              type="text"
              value={editName}
            ...
            >
              <div className={`w-5 h-5 rounded border-2 flex items-center justify-center shrink-0 transition-all ${editVisible ? 'bg-primario border-primario' : 'border-zinc-600'}`}>
                {editVisible && <span className="text-texto-en-acento text-xs font-black">✓</span>}
              </div>
              <div>
                <p className="text-texto-principal text-sm font-semibold">Aparecer en el leaderboard</p>
            ...
            <button
              ...
              className="w-full py-3 rounded-xl bg-primario hover:bg-primario-hover text-texto-en-acento font-bold text-sm transition-all disabled:opacity-40 active:scale-95"
            >
...
function StatCard({ label, points, rank }: { label: string; points: number; rank: number | null }) {
  return (
    <div className="bg-superficie rounded-2xl border border-zinc-800/60 p-4">
      ...
      <p className="text-primario font-black text-3xl leading-none">{points}</p>
...
function SendRow({ send }: { send: MySend }) {
  ...
  return (
    <div className="flex items-center gap-3 bg-superficie rounded-xl border border-zinc-800/50 px-4 py-3">
